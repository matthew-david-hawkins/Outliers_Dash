...
def my_fx(x_data, y_data, fx):
    ...
    return(x_data_fit, y_data_fit,fit_equation)
...
app.layout = html.Div(children=[
   ...
   html.Div([
    ...
    html.Div([
        html.H4(
                children='Curve Fit Applied Only To Data Between Red Lines',
            )
    ], className='row justify-content-center'),
...
def new_graph(df, fit_select, x_axis, y_axis, x_value_list, y_value_list):
    ...
    (x_fit, y_fit, fit_equation) = my_fx(x_axis, y_axis, fit_select)
...
def update_graph(selection, jsonified_data, x_value_list, y_value_list):
    ...
    x_slider_reading = '{}'.format(x_value_list[0]) + ' to ' + '{}'.format(x_value_list[1])
    y_slider_reading = html.P(children=['{}'.format(y_value_list[1]),html.Br(), ' to ', html.Br(),'{}'.format(y_value_list[0])])
